...
- Nao altere nenhum arquivo antes de concluir a Fase 2 e receber confirmacao explicita do usuario.
...
1. Leia `references/project-analysis.md` e `references/agents/project-analyzer.md`.
...
5. Imprima um resumo no formato:
...
1. Leia `references/anti-pattern-catalog.md`, `references/audit-report-template.md`, `references/agents/anti-pattern-auditor.md` e `references/agents/audit-reporter.md`.
...
6. Salve o relatorio em `reports-folder/audit-[project-name].md` quando um destino estiver disponivel.
7. Pause obrigatoriamente:
...
1. Leia `references/mvc-guidelines.md`, `references/refactoring-playbook.md`, `references/validation-checklist.md` e os perfis `refactoring-planner`, `refactoring-task-writer`, `mvc-refactor-implementer` e `refactoring-validator`.
2. Gere um plano de refatoracao baseado nos findings aprovados e na arquitetura alvo.
3. Quebre o plano em tarefas pequenas, ordenadas por menor risco: configuracao, models/repositories, controllers/services, routes/views, middlewares, bootstrap e validacao.
4. Implemente uma tarefa por vez, verificando imports, caminhos, inicializacao e compatibilidade dos endpoints.
5. Extraia configuracao sensivel para variaveis de ambiente ou modulo de configuracao com defaults seguros para desenvolvimento.
...
7. Valide boot da aplicacao, endpoints originais e reducao dos anti-patterns encontrados.
8. Gere um resumo final com nova estrutura, comandos executados e limitacoes.

## Recuperacao

Se o fluxo for interrompido:

1. Leia o relatorio de auditoria existente e qualquer lista de tarefas criada.
2. Retome da primeira fase incompleta.
3. Nao reexecute refatoracoes ja aplicadas sem conferir o diff atual.
4. Se a confirmacao da Fase 2 nao estiver registrada na conversa, volte a pedir confirmacao antes de editar.
